...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, conv_builder, stride=1, downsample=None, mhsa=False, n_frames_last_layer=2,
                 input_last_layer=(7, 7), daft=False):
        ...
        self.daft = None
        if daft:
            self.daft = DAFT(tab_dim=6, frame_dim=n_frames_last_layer)
...
class VideoResNet(nn.Module):
    ...
    def __init__(self, block, conv_makers, layers, stem, num_classes=1, zero_init_residual=False, head=False,
                 msha=False, n_frames_last_layer=2, input_last_layer=(7, 7), daft=True):
    ...
    def _make_layer(self, block, conv_builder, planes, blocks, stride=1, mhsa=False, n_frames_last_layer=2,
        ...
        layers = []
        layers.append(block(self.inplanes, planes, conv_builder, stride, downsample, mhsa=mhsa, daft=daft,
        ...
        for i in range(1, blocks):
            layers.append(
                block(self.inplanes, planes, conv_builder, mhsa=mhsa, n_frames_last_layer=n_frames_last_layer,
                      input_last_layer=input_last_layer, daft=daft))
...
def r3d_18(pretrained=False, progress=True, msha=False, n_frames=16, input_size=(224, 224), daft=True, **kwargs):
    ...
    return _video_resnet('r3d_18',
                         ...
                         input_last_layer=input_last_layer, daft=daft, **kwargs)
